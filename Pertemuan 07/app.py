...
def tambah():
    if request.method == 'POST':
        ...
        sql = "INSERT INTO barang (kode, nama, harga, jumlah) VALUES (%s, %s, %s, %s)"
...
def edit(id):
    ...
    if request.method == 'POST':
        ...
        harga = request.form['harga']
        jumlah = request.form['jumlah']
        sql = "UPDATE barang SET kode=%s, nama=%s, harga=%s, jumlah=%s WHERE id=%s"
...
# Fungsi view untuk menghapus data
...
def delete(id):
    ...
    conn.commit()
    closeDb()
    return jsonify(success=True)  # Mengembalikan respons JSON
